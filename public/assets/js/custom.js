var api_url = "52.5.44.163:9191/api";
var base_url = "";
...
var TableManaged = (function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.state == "success") {
                        toastr[response.state](
                            "Ballot activation state is changed successfully."
                        );
                    } else {
                ...
                success: function (response) {
                    ...
                    table.dataTable({
                        language: {
                            ...
                            infoEmpty: "No entries found",
                            infoFiltered:
                                "(filtered1 from _MAX_ total entries)",
                            lengthMenu: "Show _MENU_ entries",
                            search: "Search:",
                            zeroRecords: "No matching records found",
                        },
                    ...
                    $(".printBallot").on("click", function () {
                        let ballot_id = $(this).attr("data-id");
                        let ballot_title = $(this)
                            .parent()
                            .prev()
                            .prev()
                            .prev()
                        ...
                        fetch(
                            ...
                            .then((data) => {
                                ...
                                candidates.forEach((item) => {
                                    if (
                                        !race_ids.includes(item.race_id) &&
                                        item.race_title
                                    ) {
                                        race_ids.push(item.race_id);
                                        race_titles.push(item.race_title);
                                    }
                                });

                                race_ids.forEach((race_id) => {
                                    let cands = candidates.filter((item) => {
                                        return item.race_id == race_id;
                                    });
                                    new_cands[race_id] = cands;
                                });

                                race_ids.forEach((race_id) => {
                                    let cands = new_cands[race_id];
                                    let pins = [];
                                    let names = [];
                                    let race_cands = [];

                                    cands.forEach((cand) => {
                                        if (!pins.includes(cand.pincode)) {
                                            pins.push(cand.pincode);
                                        }
                                    });
                                    pincodes[race_id] = pins;

                                    cands.forEach((cand) => {
                                        if (
                                            !names.includes(cand.candidate_name)
                                        ) {
                                            names.push(cand.candidate_name);
                                        }
                                    });
                                    cand_names[race_id] = names;
                                    let all_sum = 0;
                                    ...
                                    names.forEach((name) => {
                                        ...
                                        pins.forEach((pin) => {
                                            ...
                                            let cand_elem = 0;
                                            if (filter_cands.length > 0) {
                                                cand_elem =
                                                    filter_cands[0].race_type ==
                                                    "R"
                                                        ? filter_cands[0]
                                                              .cast_value
                                                        : 1;
                                            ...
                                            sum += cand_elem;
                                            name_cands.push(cand_elem);
                                        });
                                        all_sum += sum;
                                        name_cands.push(sum);
                                        race_cands.push(name_cands);
                                    });
                                    race_cands.forEach((name_cand) => {
                                        name_cand.push(
                                            Math.round(
                                                (name_cand[
                                                    name_cand.length - 1
                                    ...
                                    res_cands.push(race_cands);
                                });
                                res_cands.forEach((race_cands) => {
                                    race_cands.sort((cand1, cand2) => {
                                        return (
                                            cand2[cand2.length - 2] -
                                            cand1[cand1.length - 2]
                                        );
                                    });

                                    race_cands.forEach((cand, index) => {
                                        race_cands[index].push(index + 1);
                                    });
                                });
                                ...
                                res_cands.forEach((race_cands, index) => {
                                    csvContent += race_titles[index] + "\r\n";
                                    let first_line = [""].concat(
                                        pincodes[race_ids[index]],
                                        ["total", "percentage", "rank"]
                                    );
                                    csvContent += first_line.join(",") + "\r\n";
                                    race_cands.forEach(function (rowArray) {
    ...
    $(".printBallot").on("click", function () {
        ...
        let ballot_title = $(this)
            ...
            .text()
            .trim();
        fetch("https://crossorigin.me/http://" + api_url + "/result/candidate?ballot_id=" + ballot_id)
            .then((res) => {
                return res.json();
            })
            .then((data) => {
                ...
                let cand_names = {};
                candidates.forEach((item) => {
                    if (!race_ids.includes(item.race_id) && item.race_title) {
                        race_ids.push(item.race_id);
                        race_titles.push(item.race_title);
                    }
                });
                ...
                race_ids.forEach((race_id) => {
                    ...
                    cands.forEach((cand) => {
                        if (!names.includes(cand.candidate_name)) {
                            names.push(cand.candidate_name);
                        }
                    });
                    cand_names[race_id] = names;
                    ...
                    names.forEach((name) => {
                        ...
                        let sum = 0;
                        pins.forEach((pin) => {
                            let filter_cands = cands.filter((cand) => {
                                return (
                                    cand.candidate_name == name &&
                                    cand.pincode == pin
                                );
                            });
                            ...
                            if (filter_cands.length > 0) {
                                cand_elem =
                                    filter_cands[0].race_type == "R"
                                        ? filter_cands[0].cast_value
                                        : 1;
                            }
                            sum += cand_elem;
                            name_cands.push(cand_elem);
                    ...
                    race_cands.forEach((name_cand) => {
                        name_cand.push(
                            Math.round(
                                (name_cand[name_cand.length - 1] * 100) /
                                    all_sum
                            ) + "%"
                        );
                    });
                ...
                res_cands.forEach((race_cands) => {
                    race_cands.sort((cand1, cand2) => {
                        return (
                            cand2[cand2.length - 2] - cand1[cand1.length - 2]
                        );
                    });
                ...
                res_cands.forEach((race_cands, index) => {
                    csvContent += race_titles[index] + "\r\n";
                    let first_line = [""].concat(pincodes[race_ids[index]], [
                        "total",
                        "percentage",
                        "rank",
                    ]);
                ...
                var link = document.createElement("a");
                link.setAttribute("href", encodedUri);
                link.setAttribute("download", ballot_title + ".csv");
                document.body.appendChild(link); // Required for FF
        ...
        table.on("click", ".previewRaceModal", function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#min_num_of_votes")
                        .val(race.data[0].min_num_of_votes);
                    modal
                        .find("#max_num_of_votes")
                        .val(race.data[0].max_num_of_votes);
                    modal
                        .find("#max_num_of_write_ins")
                        .val(race.data[0].max_num_of_write_ins);
                    if (race.data[0].race_lang_id == null) {
                        modal.find("#race_lang_id").val(0);
                    } else {
                        modal
                            .find("#race_lang_id")
                            .val(race.data[0].race_lang_id);
                    }
                    if (race.data[0].race_location_id == null) {
                        modal.find("#race_location_id").val(0);
                    } else {
                        modal
                            .find("#race_location_id")
                            .val(race.data[0].race_location_id);
                    }
                    modal.find("#race_title").val(race.data[0].race_title);
                    modal
                        .find("#race_voted_position")
                        .val(race.data[0].race_voted_position);
                },
            });
        ...
        table.$(".editRaceModal").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#edit_min_num_of_votes")
                        .val(race.data[0].min_num_of_votes);
                    modal
                        .find("#edit_max_num_of_votes")
                        .val(race.data[0].max_num_of_votes);
                    modal
                        .find("#edit_max_num_of_write_ins")
                        .val(race.data[0].max_num_of_write_ins);
                    if (race.data[0].race_lang_id == null) {
                        modal.find("#race_lang_id").val(0);
                    } else {
                    ...
                    table.on("click", ".previewRaceModal", function () {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                modal
                                    .find("#race_name")
                                    .val(race.data[0].race_name);
                                modal
                                    .find("#race_type")
                                    .val(race.data[0].race_type);
                                modal
                                    .find("#min_num_of_votes")
                                    .val(race.data[0].min_num_of_votes);
                                modal
    ...
    var candidateTable = function () {
        ...
        $(".addCandidateModal").click(function () {
            ...
            if (ballot_id == null) {
                confirmModal
                    .find(".modal_content")
                    .text(
                        "There aren't any ballots. Please make the ballot, first."
                    );
                ...
                confirmModal
                    .find(".modal_content")
                    .text(
                        "There aren't any races in this ballot. Please make the race, first."
                    );
                confirmModal.modal("show");
                ...
                confirmModal
                    .find(".modal_content")
                    .text(
                        "There aren't any parties in this ballot. Please make the party, first."
                    );
                confirmModal.modal("show");
                ...
                confirmModal
                    .find(".modal_content")
                    .text(
                        "There aren't any languages in this ballot. Please make the language, first."
                    );
                confirmModal.modal("show");
            } else {
                addModal.modal("show");
        ...
        confirmModal.find(".modal_hide").click(function () {
            ...
            confirmModal
                .find(".modal_content")
                .text("Please select porpositions.");
        });
        ...
        table.$(".previewCandidateModal").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#prev_candidate_name")
                        .val(cand.data[0].candidate_name);

                    if (cand.data[0].photo == null) {
                        modal.find("#prev_cand_photo").text("");
                    } else {
                        modal
                            .find("#prev_cand_photo")
                            .attr("src", "" + cand.data[0].photo);
                    }
                    ...
                    if (cand.data[0].party_logo == null) {
                        ...
                        modal
                            .find("#prev_party_logo")
                            .attr("src", "" + cand.data[0].party_logo);
                    }
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                                ...
                                success: function (response) {
                                    ...
                                    if ($.isEmptyObject(races)) {
                                        ...
                                        for (var x in races.data) {
                                            race_options +=
                                                "<option value=" +
                                                races.data[x]["race_id"] +
                                                ">" +
                                                races.data[x]["race_name"] +
                                                "</opiton>";
                                        }
                                    }
                                    ...
                                    if ($.isEmptyObject(langs)) {
                                        ...
                                        for (var x in langs.data) {
                                            lang_options +=
                                                "<option value=" +
                                                langs.data[x]["lang_id"] +
                                                ">" +
                                                langs.data[x]["language_name"] +
                                                "</opiton>";
                                        }
                                    }
                            ...
                            $(".del_photo").change(function () {
                                var checked = jQuery(this).is(":checked");
                                console.log(checked);
                                $("#editCandidateModal #edit_del_photo").val(
                                    checked
                                );
                            });
                                ...
                                .click(function () {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (response) {
                                            ...
                                            modal
                                                .find("#email")
                                                .val(cand.data[0].email);
                                            modal
                                                .find("#party_id")
                                                .val(cand.data[0].party_id);
                                        },
                                    });
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        ...
                                        modal
                                            ...
                                            .val(cand.data[0].candidate_name);
                                        modal
                                            .find("#edit_cand_photo")
                                            .val(cand.data[0].photo);
                                        modal
                                            .find("#edit_email")
                                            .val(cand.data[0].email);
                                        modal
                ...
                success: function (response) {
                    ...
                    if (response.state == "success") {
                        if (response.message == "0") {
                            toastr["warning"](
                                "Warning! This language have already deleted."
                            );
                        } else {
                            if (avaliable == "true") {
                                toastr["success"](
                                    "Success! Add is successfully."
                                );
                            } else if (avaliable == "false") {
                                toastr["info"](
                                    "Success! Deleted is successfully."
                                );
                            }
                        }
                    } else {
                        toastr["warning"](
                            "Warning! This language have already added."
                        );
                    }
                },
            });
                ...
                success: function (response) {
                    ...
                    if (response.state == "success") {
                        if (response.message == "-1") {
                            ...
                            toastr["info"](
                                "All languages is deleted in this ballot successfully."
                            );
                        } else if (response.message == "-2") {
                            toastr["warning"]("Have already added all.");
                        } else {
                            toastr["success"](
                                "All languages is added in this ballot successfully."
                            );
                        }
                    } else {
                        toastr["error"]("Whoops! Something went wrong.");
                ...
                success: function (response) {
                    ...
                    $(document).on(
                        ...
                        function () {
                            ...
                            var checkedAll = $(".select_all_ballot_lang").is(
                                ":checked"
                            );
                            if (checkedAll) {
                                $(this).attr("checked", true);
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    $(".save_all_ballot_lang").removeClass(
                                        "disabled"
                                    );
                                    var response = JSON.parse(response);
                                    if (response.state == "success") {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.state == "success") {
                        if (response.message == "0") {
                            toastr["warning"](
                                "Warning! This county have already deleted."
                            );
                        } else {
                            if (avaliable == "true") {
                                toastr["success"](
                        ...
                        toastr["warning"](
                            "Warning! This county have already added."
                        );
                    }
                },
            });
                ...
                success: function (response) {
                    ...
                    if (response.state == "success") {
                        if (response.message == "-1") {
                            ...
                            toastr["info"](
                                "All counties is deleted in this ballot successfully."
                            );
                        } else if (response.message == "-2") {
                            toastr["warning"]("Have already added all.");
                        } else {
                            toastr["success"](
                                "All counties is added in this ballot successfully."
                            );
                        }
                    } else {
                        toastr["error"]("Whoops! Something went wrong.");
        ...
        $("#county_state_option").on("change", function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $(document).on(
                        "click",
                        ".save_all_ballot_county_",
                        function () {
                            var allVals = [];
                            $(".save_all_ballot_county_").addClass("disabled");
                            list.find(".county_check:checked").each(
                                function () {
                                    allVals.push($(this).attr("data-id"));
                                }
                            );
                            ...
                            var checkedAll = $(".select_all_ballot_county").is(
                                ":checked"
                            );
                            if (checkedAll) {
                                $(this).attr("checked", true);
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    $(".save_all_ballot_county_").removeClass(
                                        "disabled"
                                    );
                                    var response = JSON.parse(response);
                                    if (response.state == "success") {
        ...
        table.$(".previewPropositionModal").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#prev_prop_lang_id")
                        .val(prop.data[0].prop_lang_id);
                    modal
                        .find("#prev_prop_answer_type")
                        .val(prop.data[0].prop_answer_type);
                    modal
                        .find("#prev_prop_location_id")
                        .val(prop.data[0].prop_location_id);
        ...
        table.$(".editPropositionModal").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#edit_prop_lang_id")
                        .val(prop.data[0].prop_lang_id);
                    modal
                        .find("#edit_prop_answer_type")
                        .val(prop.data[0].prop_answer_type);
                    modal
                        .find("#edit_prop_location_id")
                        .val(prop.data[0].prop_location_id);
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                modal
                                    ...
                                    .val(prop.data[0].prop_location_id);
                                modal
                                    .find("#prev_prop_title")
                                    .val(prop.data[0].prop_title);
                                modal
                                    .find("#prev_prop_text")
                                    .val(prop.data[0].prop_text);
                            },
                        });
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (user.data[0].user_avatar == null) {
                        ...
                        $("#preview_user_avatar").attr(
                            "src",
                            "" + user.data[0].user_avatar
                        );
                    }
                },
            });
        ...
        table.on("click", ".editUserModal", function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#edit_display_name")
                        .val(user.data[0].display_name);
                    modal.find("#edit_user_email").val(user.data[0].user_email);
                    modal
                        .find("#edit_user_avatar")
                        .attr("src", "" + user.data[0].user_avatar);
                },
            });
        ...
        table.$(".previewPartyModal").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#party_logo")
                        .attr("src", "" + party.data[0].party_logo);
                },
            });
        ...
        table.$(".editPartyModal").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modal
                        .find("#edit_party_name")
                        .val(party.data[0].party_name);
                    modal
                        .find("#edit_party_logo")
                        .attr("src", "" + party.data[0].party_logo);
                },
            });
