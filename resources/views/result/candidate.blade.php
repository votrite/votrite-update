				...
				<div class="portlet box blue">
					...
					<div class="portlet-body">
							...
							<div class="row">
								<div class="col-md-6 select_options">
									<div class="col-md-4 form-group">
										...
										<div class="col-sm-9">
											<select class="form-control" name="cand_ballot_name" id="result_cand_ballot_name1">
											@if(empty($ballots->data))
												<option value="-1">No Ballot</opiton>
											@else
												@foreach($ballots->data as $key => $ballot)
												<option value="{{ $key }}">{{ $ballot->election }}</opiton>
												@endforeach
											@endif
											</select>
										</div>
						...
						<form class="guide-desc-body race-voter scroller" method="post" action="{{ url('client/cast') }}" style="height: 600px; width:400px;">
							@csrf
						@foreach($ballots->data as $k => $v)
						<div id="balresult" class="balresult balresult{{$k}}" style="padding:10px; width:400px;display:<?php echo $k == 0 ? 'block' : 'none' ?>">
                        	<h4 style="font-weight: bold;" id="ballot_board">{{$ballots->data[$k]->board}}</h4>
							<h4 style="font-weight: bold;" >{{$ballots->data[$k]->address}}</h4>
                        	<h4 style="font-weight: bold;" >{{$ballots->data[$k]->client}}</h4>
                        	<h4 style="font-weight: bold;" >Start : {{date_format(date_create($ballots->data[$k]->start_date),"D M j Y")}}</h4>
                        	<h4 style="font-weight: bold;" >End : {{date_format(date_create($ballots->data[$k]->end_date),"D M j Y")}}</h4>
							<br>
							<h3 class="text-center" style="width: 400px;border-bottom:1px solid;font-weight: bold;">END OF DAY - REPORT</h3>
							<h4 style="font-weight: bold;" >TOTAL VOTES ON MACHINE........{{$blt_cnt[$k]}}</h4>
							<hr style="border: 1px solid;width: 400px;">
                            <div id="countresult" class="form-group" style="margin-left:25px;">
                            @if(count($candidates[$k]) != 0)
								@foreach($candidates[$k] as $race)
									<h4 style="font-weight: bold;">{{$race[0]['candidate']['race_title']}}</h4>
									<h4>{{$race[0]['candidate']['race_type'] == 'R' ? '--1st Preferences--' : ''}}</h4>
									{{-- <h4 style="font-weight: bold;">{{$race[0]['race_name']}}</h4> --}}
									@foreach($race as $key => $cand)
									<h4>{{$key + 1}}. {{$cand['candidate']['candidate_name']}} - {{$cand['count']}} {{$cand['candidate']['race_type'] == 'R' ? round($cand['percent']).'%' : ''}}</h4>
									@endforeach
								@endforeach
                            @else
                            	No Candidate
                            ...
                            @if(count(get_object_vars($props[$k])) != 0 && property_exists($props[$k], "data"))
								@if(count($props[$k]->data) != 0)
									@foreach($props[$k]->data as $pro)
										<h4>{{$pro->prop_name}}</h4>
...
<script>
	...
	$('#result_cand_ballot_name1').change(function(){
		const option_id = $(this).val();

		$('.balresult').css('display', 'none');
		$('.balresult' + option_id).css('display', 'block');
