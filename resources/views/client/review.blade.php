		...
		<div class="page-content-fullwidth" style="margin-top: 120px;">
            <div class="col-md-4 col-xs-12">
                ...
                <div class="guide-desc-body" data-a11y-instructions>
                    <h4>You can see the result you have made so far. Read each name and review each selection carefully. When you are done, press J or the "Cast my vote" button. If you wish to change a selection, move to a contest name and press Enter or F to edit it. Press S to hear your selections read aloud.</h4>
                </div>
            </div>
            <div class="col-md-6 col-xs-12">
                <div class="guide-desc-header">
                    <h2 data-a11y-heading tabindex="-1">Review your choices.</h2>
                </div>
                    ...
                    <div class="form-group row" style="margin-left:25px;">
                        ...
                        @if(count($totalrace) != 0)
                            @foreach($totalrace as $rkey=>$race)
                                @if($race['race_type'] != "R")
                                    <div  style="margin-left: 10px;"><h4><button type="button" class="a11y-linkbtn" onclick="gorace({{$rkey}})" aria-label="Edit contest: {{$race['race_title']}}">Candidates For: {{$race['race_title']}}.</button></h4></div>
                        ...
                        @foreach(session('props') as $key=>$prop)
                            <div  style="margin-left: 10px;"><h4><button type="button" class="a11y-linkbtn" onclick="goprop()" aria-label="Edit proposition: {{$prop->prop_title}}"> {{$prop->prop_title}}</button></h4></div>
                            <div class="form-group row" style="margin-left: 10px;">
                        ...
                        @foreach(session('mass') as $key=>$prop)
                            <div  style="margin-left: 10px;"><h4><button type="button" class="a11y-linkbtn" onclick="gomass()" aria-label="Edit proposition: {{$prop->prop_title}}"> {{$prop->prop_title}}</button></h4></div>
                            <div class="form-group row" style="margin-left: 10px;">
                                <div class="col-md-3 col-xs-6">
                                    <h4>
                                    {{$prop->prop_text}}
...
<div class="page-footer-voter" style="text-align:center; padding-top: 35px; color:white; position: absolute; width: 100%;">
	<div class="col-md-3 col-xs-3" style="padding: 0px;">
        <button type="button" class="btn-review" data-a11y="back" onclick="govote(<?php echo session('current')-1?>)">Go Back</button>
    </div>
    <div class="col-md-3 col-xs-3" style="padding: 0px;">
    ...
    <div class="col-md-3 col-xs-3" style="padding: 0px;">
        <button type="button" class="btn-voter" data-a11y="next">Cast my vote</button>
	</div>
</div>
@endsection
@section('script')
